...
const UsersDetails = () => {
  ...
  const { id } = useParams();
  useEffect(() => {
    fetch(`https://api.eaglevisionmri.com/api/v1/users/${id}`, {
      headers: {
      ...
      .then((data) => {
        ...
        fetch(
          `https://api.eaglevisionmri.com/api/v1/transactions/getAllTransactionsByCollector?collectedBy=${data.data.lastName}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
